...
describe("mcpi ema helpers", () => {
  let dir: string;
  let savedEnv: Record<string, string | undefined>;

  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "mcpi-ema-"));
    savedEnv = {
  ...
  it("getEmaStatus reports unconfigured when client.json has no EMA block", async () => {
    const { getEmaStatus } = await import("../src/session/ema.js");
  ...
  it("getEmaStatus reports a disabled config (still shows issuer + session state)", async () => {
  ...
  it("emaLogin fails with actionable guidance when EMA is not configured", async () => {
    const { emaLogin } = await import("../src/session/ema.js");
  ...
  it("emaLogout fails when EMA is not configured", async () => {
    const { emaLogout } = await import("../src/session/ema.js");
  ...
  it("emaLogout works even when EMA is disabled, and clears the IdP session", async () => {
    writeClientConfig(emaClientConfig(false));
    await seedIdpSession();
    const { emaLogout, getEmaStatus } = await import("../src/session/ema.js");
  ...
  it("emaLogin runs the IdP flow via the runner adapter and reports the new session", async () => {
    writeClientConfig(emaClientConfig());
    let stderr = "";
  ...
  it("emaLogin --relogin clears the existing session and re-runs the flow", async () => {
    writeClientConfig(emaClientConfig());
    await seedIdpSession();
    startIdpOidcAuthorization.mockResolvedValue({
    ...
    runRunnerInteractiveOAuth.mockImplementation(
      async (options: {
        ...
        // The pre-existing session must already be gone before leg 1 runs.
        const storage = new NodeOAuthStorage();
  ...
  it("mcpiEmaGuidance names both configuration routes", async () => {
    const { mcpiEmaGuidance } = await import("../src/session/ema.js");
    expect(mcpiEmaGuidance("not_configured")).toMatch(
      /Client Settings.*enterpriseManagedAuth/is,
    );
    expect(mcpiEmaGuidance("disabled")).toContain("enabled");
  });
});
