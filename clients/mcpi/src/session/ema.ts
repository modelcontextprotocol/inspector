...
/**
 * mcpi-flavoured guidance for a missing/disabled EMA client configuration.
 * The core `EmaClientNotConfiguredError` message points at the web Client
 * Settings dialog; mcpi users may equally well edit `client.json` directly,
 * so name both, with the resolved path.
 */
export function mcpiEmaGuidance(reason: EmaClientNotConfiguredReason): string {
...
function requireIdp(
  ...
  if (!idp) {
    throw new CliExitCodeError(
      EXIT_CODES.USAGE,
      mcpiEmaGuidance("not_configured"),
      {
        code: "usage",
      },
    );
  ...
  if (!enabled && !options?.allowDisabled) {
    throw new CliExitCodeError(EXIT_CODES.USAGE, mcpiEmaGuidance("disabled"), {
      code: "usage",
    });
  }
  return idp;
...
/**
 * Sign out of the enterprise IdP: clears the cached IdP OIDC session and all
 ...
 * tokens silently from this session.
 *
