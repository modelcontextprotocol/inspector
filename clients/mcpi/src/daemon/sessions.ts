...
import type { SessionAuthInfo, SessionInfo } from "./protocol.js";
...
type LiveSession = {
  ...
  /** Retained for `sessions/show`'s live auth recompute. */
  serverConfig: MCPServerConfig;
  serverSettings?: InspectorServerSettings;
  /** Connect-time snapshot (see {@link SessionInfo.auth}). */
...
export class SessionRegistry {
  ...
  async connect(params: {
    ...
    try {
      ...
      const now = Date.now();
      const auth = await getSessionAuthInfo(client);
...
export function isSessionAuthRequiredError(error: unknown): boolean {
  ...
  // actionable mcpi guidance, instead of this daemon relaying the web-centric
 ...
 * {@link SessionAuthInfo} reported on `SessionInfo`.
 */
function projectAuthState(state: OAuthConnectionState): SessionAuthInfo {
  return {
 ...
 * (`getOAuthState()` returns undefined for both), so no-auth sessions simply
...
export async function getSessionAuthInfo(
  client: InspectorClient,
): Promise<SessionAuthInfo | undefined> {
  let state;
  try {
...
/**
 * Live auth snapshot for `sessions/show`, read from *disk* rather than the
 ...
 * semantics (undefined).
 */
export async function getLiveSessionAuthInfo(session: {
  serverConfig: MCPServerConfig;
  serverSettings?: InspectorServerSettings;
}): Promise<SessionAuthInfo | undefined> {
  try {
