 ...
 * daemon.
 *
 * Lives under `$TMPDIR/mcpi-<uid>/<id>/`, not `~/.mcp-inspector`: `sun_path`
 ...
 * `mcpi-<uid>` dir is also created 0700 so the layout never depends on the
...
export function createPrivateDaemonDir(): string {
  ...
  const root = path.join(os.tmpdir(), `mcpi-${uid}`);
...
export function assertSocketPathWithinLimit(socketPath: string): void {
  ...
  if (bytes > limit) {
    throw new Error(
      `Session daemon socket path is too long for this platform ` +
