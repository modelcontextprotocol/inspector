...
/**
 * Slim connect-time snapshot of a session's authorization, projected from the
 * core `OAuthConnectionState` (see {@link SessionInfo.auth}). Absent entirely
...
export type SessionInfo = {
   ...
   * session is reported so both humans and agents can see *how* a session is
   ...
   * connection succeeds; `sessions/list` and `sessions/use` reuse that
   * connect-time value; `sessions/show` recomputes it live *from disk* so it
   * reflects the current persisted state (e.g. after `auth/clear` or
   * `auth/ema-logout`, even from another process). Note a live session may
