    ...
    <Card withBorder padding="md">
      <Stack gap="sm">
        <Group justify="space-between" wrap="nowrap">
          <Group gap="sm">
            <Text size="sm" c="dimmed" ff="monospace">{timestamp}</Text>
            <Badge color="dark">{method}</Badge>
            {target && <Text size="sm" fw={500}>{target}</Text>}
          </Group>
          <Group gap="sm">
            <Badge color={status === 'success' ? 'green' : 'red'}>
              {status === 'success' ? 'OK' : 'Error'}
            </Badge>
            <Text size="sm" c="dimmed">{durationMs}ms</Text>
          </Group>
        </Group>
              ...
              <Stack gap="xs">
                {children.map((child, index) => (
                  <Group key={index} pl="lg" gap="sm">
                    ...
                    <Text size="sm" c="dimmed" ff="monospace">{child.timestamp}</Text>
                    <Badge color="dark" size="sm">{child.method}</Badge>
                    {child.target && <Text size="sm">{child.target}</Text>}
                    <Badge color={child.status === 'success' ? 'green' : 'red'} size="sm">
                      {child.status === 'success' ? 'OK' : 'Error'}
                    </Badge>
                    <Text size="sm" c="dimmed">{child.durationMs}ms</Text>
                  </Group>
                ))}
              </Stack>
            )}
