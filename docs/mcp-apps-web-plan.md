...
- **Web server in TypeScript:** The app server lives in `web/src/server.ts` (TypeScript), built with `tsc -p tsconfig.server.json` and emitted as `dist/server.js`. The web entrypoint is `web/build/index.js` (from `web/src/web.ts`), which parses argv and spawns Vite (dev) or `dist/server.js` (prod).
