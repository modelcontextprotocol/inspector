...
- The image runs as non-root and has a `HEALTHCHECK` that probes the web UI under `--web`; under `--cli` / `--tui`, which have no web server, it reports the container healthy while it runs, so no `--no-healthcheck` is needed.
