...
**Where secrets go, and how to make them survive (#1950).** The Inspector keeps acquired OAuth tokens, OAuth client secrets, the enterprise IdP client secret and each stdio `env:` value out of `mcp.json` and `oauth.json`, in the OS keychain. A container has no keychain (the published image has no D-Bus session), so it falls back, and which fallback you get depends on whether the secrets directory is going to survive:
