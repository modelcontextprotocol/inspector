    ...
    return async (c, next) => {
        ...
        if (c.req.method === "OPTIONS") {
            if (origin && allowedOrigins.includes(origin)) {
                ...
                c.header("Access-Control-Allow-Headers", "Content-Type, x-mcp-remote-auth");
...
function buildInitialConfigFromEnv() {
    const defaultEnvKeys = process.platform === "win32"
        ? [
            "APPDATA",
            "HOMEDRIVE",
            "HOMEPATH",
...
function forwardLogEvent(logger, logEvent) {
    ...
    if (typeof method !== "function")
        return;
    const bindings = Object.assign({}, ...(Array.isArray(logEvent.bindings) ? logEvent.bindings : []));
    ...
    if (typeof first === "object" && first !== null && !Array.isArray(first)) {
        ...
        method.call(logger, obj, msg, ...args);
    }
    else {
        const msg = messages[0];
        const args = messages.slice(1);
        method.call(logger, bindings, msg, ...args);
    }
}
export function createRemoteApp(options = {}) {
    ...
    app.post("/api/mcp/connect", async (c) => {
        ...
        try {
            ...
            return c.json({ error: `Failed to start transport: ${msg}` }, is401 ? 401 : 500);
        }
