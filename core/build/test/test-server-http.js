...
import { setupOAuthRoutes, createBearerTokenMiddleware, } from "./test-server-oauth.js";
import { setTestServerControl, } from "./test-server-control.js";
/**
...
export class TestServerHttp {
     ...
     */
    setupMessageInterception(transport) {
        ...
        transport.onmessage = async (message) => {
            ...
            const metadata = params && typeof params === "object" && "_meta" in params
                ? params._meta
                : undefined;
            try {
    ...
    async startHttp(port) {
        ...
        app.get("/mcp", ...mcpMiddleware, async (req, res) => {
    ...
    async startSse(port) {
        ...
        app.post("/sse", ...sseMiddleware, async (req, res) => {
            this.currentRequestHeaders = extractHeaders(req);
     ...
     */
    waitUntilRecorded(predicate, options) {
        ...
        return new Promise((resolve, reject) => {
            const check = () => {
                ...
                if (Date.now() - start >= timeout) {
                    reject(new Error(`Timeout (${timeout}ms) waiting for recorded request matching predicate`));
                    return;
                }
 ...
 */
export function createTestServerHttp(config) {
    return new TestServerHttp(config);
}
//# sourceMappingURL=test-server-http.js.map