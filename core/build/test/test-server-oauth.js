 ...
 */
export function setupOAuthRoutes(app, config, baseUrl) {
 ...
 */
export function createBearerTokenMiddleware(config) {
 ...
 */
function setupMetadataEndpoints(app, config, issuerUrl) {
 ...
 */
function setupAuthorizationEndpoint(app, config, issuerUrl) {
    app.get("/oauth/authorize", async (req, res) => {
        ...
        const authCode = generateAuthorizationCode(client_id, code_challenge);
 ...
 */
function setupTokenEndpoint(app, config, issuerUrl) {
    app.post("/oauth/token", express.urlencoded({ extended: true }), async (req, res) => {
        const { grant_type, code, redirect_uri, client_id: bodyClientId, code_verifier, refresh_token, } = req.body;
 ...
 */
function setupDCREndpoint(app, config, issuerUrl) {
 ...
 */
async function fetchClientMetadata(metadataUrl) {
    try {
...
async function findClient(clientId, config) {
    // Check static clients first
    if (config.staticClients) {
        const staticClient = config.staticClients.find((c) => c.clientId === clientId);
        if (staticClient) {
            return {
...
function generateAuthorizationCode(clientId, codeChallenge) {
...
function registerClient(clientId, client) {
 ...
 */
export function clearOAuthTestData() {
 ...
 */
export function getDCRRequests() {
    return dcrRequests;
}
/**
...
//# sourceMappingURL=test-server-oauth.js.map