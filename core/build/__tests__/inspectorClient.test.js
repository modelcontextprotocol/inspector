...
import { createEchoTool, createTestServerInfo, createFileResourceTemplate, createCollectSampleTool, createCollectFormElicitationTool, createCollectUrlElicitationTool, createSendNotificationTool, createListRootsTool, createArgsPrompt, createArchitectureResource, createTestCwdResource, createSimplePrompt, createUserResourceTemplate, createNumberedTools, createNumberedResources, createNumberedResourceTemplates, createNumberedPrompts, getTaskServerConfig, createElicitationTaskTool, createSamplingTaskTool, createProgressTaskTool, createFlexibleTaskTool, } from "../test/test-server-fixtures.js";
import { RELATED_TASK_META_KEY, McpError, ErrorCode, } from "@modelcontextprotocol/sdk/types.js";
describe("InspectorClient", () => {
    ...
    describe("Connection Management", () => {
        ...
        it("should clear messages on connect", async () => {
            ...
            if (firstConnectMessages.length > 0 && secondConnectMessages.length > 0) {
                const lastFirstMessage = firstConnectMessages[firstConnectMessages.length - 1];
                const firstSecondMessage = secondConnectMessages[0];
                if (lastFirstMessage && firstSecondMessage) {
                    expect(firstSecondMessage.timestamp.getTime()).toBeGreaterThanOrEqual(lastFirstMessage.timestamp.getTime());
                }
            }
        });
    });
    ...
    describe("Resource Template Methods", () => {
        ...
        it("should paginate resource templates when maxPageSize is set", async () => {
            ...
            expect(page1.nextCursor).toBeDefined();
            expect(page1.resourceTemplates[0]?.uriTemplate).toBe("test://template-1/{param}");
            expect(page1.resourceTemplates[1]?.uriTemplate).toBe("test://template-2/{param}");
            expect(page1.resourceTemplates[2]?.uriTemplate).toBe("test://template-3/{param}");
            ...
            expect(page2.nextCursor).toBeDefined();
            expect(page2.resourceTemplates[0]?.uriTemplate).toBe("test://template-4/{param}");
            expect(page2.resourceTemplates[1]?.uriTemplate).toBe("test://template-5/{param}");
            expect(page2.resourceTemplates[2]?.uriTemplate).toBe("test://template-6/{param}");
            ...
            expect(page3.nextCursor).toBeDefined();
            expect(page3.resourceTemplates[0]?.uriTemplate).toBe("test://template-7/{param}");
            expect(page3.resourceTemplates[1]?.uriTemplate).toBe("test://template-8/{param}");
            expect(page3.resourceTemplates[2]?.uriTemplate).toBe("test://template-9/{param}");
            ...
            expect(page4.nextCursor).toBeUndefined();
            expect(page4.resourceTemplates[0]?.uriTemplate).toBe("test://template-10/{param}");
        ...
        it("should accumulate resource templates when paginating with cursor", async () => {
            ...
            expect(client.getResourceTemplates()[0]?.uriTemplate).toBe("test://template-1/{param}");
            expect(client.getResourceTemplates()[1]?.uriTemplate).toBe("test://template-2/{param}");
            ...
            expect(client.getResourceTemplates()[2]?.uriTemplate).toBe("test://template-3/{param}");
            expect(client.getResourceTemplates()[3]?.uriTemplate).toBe("test://template-4/{param}");
            ...
            expect(client.getResourceTemplates()[4]?.uriTemplate).toBe("test://template-5/{param}");
            expect(client.getResourceTemplates()[5]?.uriTemplate).toBe("test://template-6/{param}");
            // Calling without cursor again should reset
    ...
    describe("Progress Tracking", () => {
        it("should dispatch progressNotification events when progress notifications are received", async () => {
            const { createSendProgressTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
        ...
        it("should not dispatch progressNotification events when progress is disabled", async () => {
            ...
            await client.callTool("sendProgress", {
                units: 2,
                delayMs: 50,
            }, undefined, // generalMetadata
            { progressToken: progressToken.toString() });
        ...
        it("should complete when timeout and resetTimeoutOnProgress are set (options passed through)", async () => {
            ...
            const result = await client.callTool("sendProgress", { units: 3, delayMs: 100, total: 3, message: "Timeout test" }, undefined, { progressToken: progressToken.toString() });
            expect(result.success).toBe(true);
        ...
        it("should not timeout when resetTimeoutOnProgress is true and progress is sent (reset extends timeout)", async () => {
            ...
            await client.connect();
            const result = await client.callTool("sendProgress", { units: 4, delayMs: 200, total: 4, message: "Reset test" }, undefined, { progressToken: "reset-test" });
            expect(result.success).toBe(true);
            expect(result.result.content).toBeDefined();
            const text = result.result.content?.find((c) => c.type === "text")?.text;
        ...
        it("should timeout with RequestTimeout when resetTimeoutOnProgress is false and gap exceeds timeout", async () => {
            ...
            let err;
            try {
                await client.callTool("sendProgress", { units: 4, delayMs: 200, total: 4, message: "Timeout test" }, undefined, { progressToken: progressToken.toString() });
            }
            catch (e) {
                err = e;
            }
    ...
    describe("Sampling Requests", () => {
        it("should handle sampling requests from server and respond", async () => {
            ...
            const samplingRequestPromise = new Promise((resolve) => {
                client.addEventListener("newPendingSample", (event) => {
            ...
            if (firstMessage &&
                ...
                expect(firstMessage.content.text).toBe("Hello, world!");
            }
            ...
            const toolContent = toolResult.result.content;
    ...
    describe("Server-Initiated Notifications", () => {
        it("should receive server-initiated notifications via stdio transport", async () => {
            ...
            if ("method" in notificationEntry.message) {
                ...
                if ("params" in notificationEntry.message) {
                    const params = notificationEntry.message.params;
        ...
        it("should receive server-initiated notifications via streamable-http transport", async () => {
            ...
            if ("method" in notificationEntry.message) {
                ...
                if ("params" in notificationEntry.message) {
                    const params = notificationEntry.message.params;
                    expect(params.data.message).toBe("Test notification from streamable-http");
    ...
    describe("Elicitation Requests", () => {
        it("should handle form-based elicitation requests from server and respond", async () => {
            ...
            const elicitationRequestPromise = new Promise((resolve) => {
            ...
            expect(pendingElicitation.request.params.message).toBe("Please provide your name");
            if ("requestedSchema" in pendingElicitation.request.params) {
                ...
                expect(pendingElicitation.request.params.requestedSchema.type).toBe("object");
            }
        ...
        it("should handle URL-based elicitation requests from server and respond", async () => {
            ...
            expect(pendingElicitation.request.method).toBe("elicitation/create");
            expect(pendingElicitation.request.params.message).toBe("Please visit the URL to complete authentication");
            expect(pendingElicitation.request.params.mode).toBe("url");
            if (pendingElicitation.request.params.mode === "url") {
                expect(pendingElicitation.request.params.url).toBe("https://example.com/auth");
                expect(pendingElicitation.request.params.elicitationId).toBe("test-url-elicitation-123");
            }
    ...
    describe("Roots Support", () => {
        ...
        it("should send roots/list_changed notification when roots are updated", async () => {
            ...
            const rootsChangedNotification = await server.waitUntilRecorded((req) => req.method === "notifications/roots/list_changed", { timeout: 5000, interval: 10 });
            expect(rootsChangedNotification.method).toBe("notifications/roots/list_changed");
            // Verify getRoots() returns the new roots
            const roots = client.getRoots();
            expect(roots).toEqual(newRoots);
            // Verify rootsChange event was dispatched
            const rootsChangePromise = new Promise((resolve) => {
            ...
            const secondNotification = updatedRequests.filter((req) => req.method === "notifications/roots/list_changed");
    ...
    describe("Completions", () => {
        it("should get completions for resource template variable", async () => {
            ...
            const completionCallback = (argName, value) => {
                if (argName === "path") {
            ...
            const result = await client.getCompletions({ type: "ref/resource", uri: "file:///{path}" }, "path", "file1");
        ...
        it("should get completions for prompt argument", async () => {
            ...
            const cityCompletions = (value, _context) => {
            ...
            const result = await client.getCompletions({ type: "ref/prompt", name: "args-prompt" }, "city", "New");
        ...
        it("should return empty array when server does not support completions", async () => {
            ...
            const result = await client.getCompletions({ type: "ref/resource", uri: "file:///{path}" }, "path", "file");
        ...
        it("should get completions with context (other arguments)", async () => {
            ...
            const stateCompletions = (value, context) => {
                const statesByCity = {
                ...
                if (city && statesByCity[city]) {
                    return statesByCity[city].filter((s) => s.toLowerCase().startsWith(value.toLowerCase()));
                }
                return ["NY", "CA", "TX", "FL"].filter((s) => s.toLowerCase().startsWith(value.toLowerCase()));
            };
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                prompts: [
            ...
            const result = await client.getCompletions({ type: "ref/prompt", name: "args-prompt" }, "state", "N", { city: "New York" });
        ...
        it("should handle async completion callbacks", async () => {
            ...
            const asyncCompletionCallback = async (argName, value) => {
            ...
            await client.connect();
            const result = await client.getCompletions({ type: "ref/resource", uri: "file:///{path}" }, "path", "async1");
    ...
    describe("ContentCache integration", () => {
        ...
        it("should cache resource content and dispatch event when readResource is called", async () => {
            ...
            client.addEventListener("resourceContentChange", (event) => {
                eventReceived = true;
        ...
        it("should cache resource template content and dispatch event when readResourceFromTemplate is called", async () => {
            ...
            let eventReceived = false;
            let eventDetail = null;
            client.addEventListener("resourceTemplateContentChange", (event) => {
                eventReceived = true;
                eventDetail = event.detail;
            }, { once: true });
            const invocation = await client.readResourceFromTemplate(template.uriTemplate, params);
            // Verify cache
        ...
        it("should cache prompt content and dispatch event when getPrompt is called", async () => {
            ...
            let eventReceived = false;
            let eventDetail = null;
            client.addEventListener("promptContentChange", (event) => {
                eventReceived = true;
                eventDetail = event.detail;
            }, { once: true });
        ...
        it("should replace cache entry on subsequent calls", async () => {
            ...
            expect(cached2?.timestamp.getTime()).toBeGreaterThan(invocation1.timestamp.getTime());
            await client.disconnect();
        });
    ...
    describe("Resource Subscriptions", () => {
        ...
        it("should clear subscriptions on disconnect", async () => {
            ...
            client.subscribedResources.add("test://uri1");
            client.subscribedResources.add("test://uri2");
    ...
    describe("ListChanged Notifications", () => {
        it("should initialize listChangedNotifications config with defaults (all enabled)", async () => {
            ...
            expect(client.listChangedNotifications).toEqual({
                tools: true,
                resources: true,
                prompts: true,
            });
        ...
        it("should update state and dispatch event when listAllTools() is called", async () => {
            ...
            const toolsChangePromise = new Promise((resolve) => {
                client.addEventListener("toolsChange", (event) => {
                    resolve(event);
                }, { once: true });
            });
        ...
        it("should update state, clean cache, and dispatch event when listResources() is called", async () => {
            ...
            const resourcesChangePromise = new Promise((resolve) => {
                client.addEventListener("resourcesChange", (event) => {
                    resolve(event);
                }, { once: true });
            });
        ...
        it("should update state, clean cache, and dispatch event when listAllResourceTemplates() is called", async () => {
            ...
            const resourceTemplatesChangePromise = new Promise((resolve) => {
                client.addEventListener("resourceTemplatesChange", (event) => {
                    resolve(event);
                }, { once: true });
            });
            const templates = await client.listAllResourceTemplates();
        ...
        it("should update state, clean cache, and dispatch event when listAllPrompts() is called", async () => {
            ...
            const promptsChangePromise = new Promise((resolve) => {
                client.addEventListener("promptsChange", (event) => {
                    resolve(event);
                }, { once: true });
            });
            const prompts = await client.listAllPrompts();
        ...
        it("should handle tools/list_changed notification and reload tools", async () => {
            const { createAddToolTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
        ...
        it("should fire toolsListChanged event when server sends tools/list_changed notification", async () => {
            ...
            const toolsListChangedPromise = new Promise((resolve) => {
        ...
        it("should handle resources/list_changed notification and reload resources and templates", async () => {
            const { createAddResourceTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                resources: [createArchitectureResource()],
                resourceTemplates: [createFileResourceTemplate()],
            ...
            expect(client.getResources().find((r) => r.uri === "test://new-resource")).toBeDefined();
            await client.disconnect();
            await server.stop();
        });
        it("should handle prompts/list_changed notification and reload prompts", async () => {
            const { createAddPromptTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                prompts: [createSimplePrompt()],
            ...
            expect(client.getPrompts().find((p) => p.name === "newPrompt")).toBeDefined();
            await client.disconnect();
            await server.stop();
        });
        ...
        it("should handle tools/list_changed notification on removal and clear tool call cache", async () => {
            const { createRemoveToolTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
        ...
        it("should handle resources/list_changed notification on removal and clear resource cache", async () => {
            const { createRemoveResourceTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                resources: [createArchitectureResource()],
        ...
        it("should handle prompts/list_changed notification on removal and clear prompt cache", async () => {
            const { createRemovePromptTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                prompts: [createSimplePrompt()],
    ...
    describe("Resource Subscriptions", () => {
        it("should subscribe to a resource and track subscription state", async () => {
            ...
            await expect(client.subscribeToResource("demo://resource/static/document/architecture.md")).rejects.toThrow("Server does not support resource subscriptions");
            await client.disconnect();
            await server.stop();
        });
        it("should subscribe to a resource when server supports subscriptions", async () => {
            const { createUpdateResourceTool } = await import("../test/test-server-fixtures.js");
            server = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                resources: [createArchitectureResource()],
        ...
        it("should throw error when unsubscribe called while not connected", async () => {
            ...
            await client.connect();
            await client.disconnect();
            await expect(client.unsubscribeFromResource("demo://resource/static/document/architecture.md")).rejects.toThrow();
            await server.stop();
        });
        it("should handle resource updated notification and clear cache for subscribed resource", async () => {
            ...
            const eventPromise = new Promise((resolve) => {
                client.addEventListener("resourceUpdated", ((event) => {
                    resolve(event);
                }), { once: true });
            });
    ...
    describe("Task Support", () => {
        ...
        it("should call tool with task support using callToolStream", async () => {
            const taskCreatedEvents = [];
            const taskStatusEvents = [];
            const taskCompletedEvents = [];
            const toolCallResultEvents = [];
        ...
        it("should throw error when calling callTool on task-required tool", async () => {
            await expect(client.callTool("simpleTask", { message: "test" })).rejects.toThrow("requires task support");
        });
        ...
        it("should handle task failure and dispatch taskFailed event", async () => {
            ...
            await client.connect();
            const failedPromise = expect(client.callToolStream("failingTask", { message: "test" })).rejects.toThrow();
            const taskFailedDetail = await waitForEvent(client, "taskFailed", { timeout: 2000 });
        ...
        it("should cancel a running task", async () => {
            ...
            const taskCreatedDetail = await waitForEvent(client, "taskCreated", { timeout: 3000 });
            const taskId = taskCreatedDetail.taskId;
            expect(taskId).toBeDefined();
            const cancelledPromise = waitForEvent(client, "taskCancelled", { timeout: 3000 });
            ...
            const cancelledDetail = cancelledResult.value;
        ...
        it("should handle elicitation with task (input_required flow)", async () => {
            ...
            await client.connect();
            const elicitationPromise = waitForEvent(client, "newPendingElicitation", { timeout: 2000 });
        ...
        it("should handle sampling with task (input_required flow)", async () => {
            ...
            await client.connect();
            const samplingPromise = waitForEvent(client, "newPendingSample", { timeout: 3000 });
            const taskCreatedPromise = waitForEvent(client, "taskCreated", { timeout: 3000 });
        ...
        it("should handle progress notifications linked to tasks", async () => {
            ...
            const taskCreatedPromise = waitForEvent(client, "taskCreated", { timeout: 5000 });
            const progressPromise = waitForProgressCount(client, 5, {
                timeout: 5000,
            });
            const taskCompletedPromise = waitForEvent(client, "taskCompleted", { timeout: 5000 });
            const resultPromise = client.callToolStream("taskWithProgress", { message: "test" }, undefined, { progressToken });
            const taskCreatedDetail = await taskCreatedPromise;
            const taskId = taskCreatedDetail.taskId;
            expect(taskId).toBeDefined();
            ...
            progressEvents.forEach((evt, index) => {
                const event = evt;
                ...
                const relatedTask = event._meta?.[RELATED_TASK_META_KEY];
...
//# sourceMappingURL=inspectorClient.test.js.map