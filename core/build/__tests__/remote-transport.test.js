...
describe("Remote transport e2e", () => {
    ...
    async function setupRemoteAndConnect(config) {
    ...
    it("smoke: remote server accepts connect and returns sessionId for SSE", async () => {
        ...
        const res = await fetch(`${baseUrl}/api/mcp/connect`, {
            ...
            body: JSON.stringify({
                config: { type: "sse", url: mcpHttpServer.url },
            }),
        });
        const json = (await res.json());
        if (!res.ok) {
            throw new Error(`Connect failed: ${res.status} ${json.error ?? (await res.text())}`);
        }
    ...
    describe("stdio", () => {
        ...
        it("validates stderr content over remote stdio", async () => {
            ...
            try {
                ...
                const stderrLogs = client.getStderrLogs();
                expect(Array.isArray(stderrLogs)).toBe(true);
                const matching = stderrLogs.filter((l) => l.message.includes(testMessage));
        ...
        it("calls a tool over remote stdio", async () => {
            ...
            try {
                ...
                const textContent = invocation.result?.content?.find((c) => c.type === "text");
                expect(textContent).toBeDefined();
                expect(textContent.text).toContain("Echo: hello-remote");
            }
            finally {
                await client.disconnect();
            }
    ...
    describe("SSE", () => {
        ...
        it("calls a tool over remote SSE", async () => {
            ...
            try {
                ...
                expect(invocation.result?.content).toBeDefined();
                const textContent = invocation.result?.content?.find((c) => c.type === "text");
                expect(textContent.text).toContain("Echo: sse-test");
            }
            finally {
                await client.disconnect();
            }
    ...
    describe("streamable-http", () => {
        ...
        it("calls a tool over remote streamable-http", async () => {
            ...
            try {
                ...
                expect(invocation.result?.content).toBeDefined();
                const textContent = invocation.result?.content?.find((c) => c.type === "text");
                expect(textContent.text).toContain("Echo: streamable-http-test");
            }
            finally {
                await client.disconnect();
            }
    ...
    describe("remote logging", () => {
        ...
        it("writes InspectorClient logs to file via createRemoteLogger over remote transport", async () => {
            ...
            const fileLogger = pino({ level: "info" }, pino.destination({ dest: logPath, append: true, mkdir: true }));
            mcpHttpServer = createTestServerHttp({
                serverInfo: createTestServerInfo(),
                tools: [createEchoTool()],
                serverType: "sse",
            ...
            const client = new InspectorClient({ type: "sse", url: mcpHttpServer.url }, {
                environment: {
                    transport: createTransport,
                    logger: remoteLogger,
                },
...
//# sourceMappingURL=remote-transport.test.js.map