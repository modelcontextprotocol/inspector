...
import { createOAuthTestServerConfig, createOAuthClientConfig, completeOAuthAuthorization, createClientMetadataServer, } from "../test/test-server-fixtures.js";
import { clearOAuthTestData, getDCRRequests, invalidateAccessToken, } from "../test/test-server-oauth.js";
import { clearAllOAuthClientState, NodeOAuthStorage, } from "../auth/node/index.js";
const oauthTestStatePath = path.join(os.tmpdir(), `mcp-oauth-${process.pid}-inspectorClient-oauth-e2e.json`);
...
describe("InspectorClient OAuth E2E", () => {
    ...
    describe.each(transports)("CIMD (Client ID Metadata Documents) Mode ($name)", (transport) => {
        let metadataServer = null;
        afterEach(async () => {
            if (metadataServer) {
    ...
    describe.each(transports)("Single redirect URL (DCR) ($name)", (transport) => {
    ...
    describe.each(transports)("Resource metadata discovery and oauthStepChange ($name)", (transport) => {
        it("should discover resource metadata and set resource in guided flow", async () => {
            ...
            expect(state?.resourceMetadata?.authorization_servers?.length).toBeGreaterThanOrEqual(1);
    ...
    describe.each(transports)("Token refresh (authProvider) ($name)", (transport) => {
    ...
    describe.each(transports)("Storage path (custom) ($name)", (transport) => {
        it("should persist OAuth state to custom storagePath", async () => {
            const customPath = path.join(os.tmpdir(), `mcp-inspector-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
            ...
            try {
                ...
                const parsed = await waitForStateFile(customPath, (p) => {
                    const servers = p?.state?.servers ?? {};
                    return Object.values(servers).some((s) => !!s?.tokens
                        ?.access_token);
                }, { timeout: 2000, interval: 50 });
                expect(Object.keys(parsed.state?.servers ?? {}).length).toBeGreaterThan(0);
            }
            finally {
                try {
    ...
    describe("fetchFn integration", () => {
        it("should use provided fetchFn for OAuth HTTP requests", async () => {
            ...
            const oauthUrls = tracker.filter((c) => c.url.includes("well-known") ||
            ...
            const transportFetches = fetchRequests.filter((r) => r.category === "transport");
            expect(authFetches.length).toBeGreaterThan(0);
            expect(transportFetches.length).toBeGreaterThan(0);
        });
    });
});
//# sourceMappingURL=inspectorClient-oauth-e2e.test.js.map