...
const oauthTestStatePath = path.join(os.tmpdir(), `mcp-oauth-${process.pid}-inspectorClient-oauth.json`);
...
describe("InspectorClient OAuth", () => {
    ...
    describe("OAuth fetch tracking", () => {
        ...
        it("should track auth fetches with category 'auth' during guided auth", async () => {
            ...
            const authFetches = fetchRequests.filter((req) => req.category === "auth");
            expect(authFetches.length).toBeGreaterThan(0);
            const hasOAuthUrls = authFetches.some((req) => req.url.includes("well-known") ||
    ...
    describe("OAuth Events", () => {
        ...
        it("should dispatch oauthAuthorizationRequired event", async () => {
            ...
            const detail = await waitForEvent(testClient, "oauthAuthorizationRequired", { timeout: 5000 });
        ...
        it("should dispatch oauthError event when OAuth flow fails", async () => {
            ...
            const detail = await waitForEvent(testClient, "oauthError", {
                timeout: 3000,
            });
    ...
    describe("Token Injection in HTTP Transports", () => {
        ...
        it("should inject Bearer token in HTTP requests when OAuth is configured", async () => {
            ...
            const authFetches = fetchRequests.filter((req) => req.category === "auth");
            expect(authFetches.length).toBeGreaterThan(0);
            const oauthFetches = authFetches.filter((req) => req.url.includes("well-known") ||
                req.url.includes("/oauth/") ||
                req.url.includes("/token"));
            expect(oauthFetches.length).toBeGreaterThan(0);
            // Transport fetches (SSE, MCP) should have category 'transport'
            const transportFetches = fetchRequests.filter((req) => req.category === "transport");
            expect(transportFetches.length).toBeGreaterThan(0);
            const mcpPostRequests = transportFetches.filter((req) => req.method === "POST" &&
...
//# sourceMappingURL=inspectorClient-oauth.test.js.map