...
import { ContentCache, } from "../mcp/contentCache.js";
// Helper functions to create test invocation objects
function createResourceReadInvocation(uri, timestamp = new Date()) {
    return {
        uri,
        timestamp,
        result: {
...
function createResourceTemplateReadInvocation(uriTemplate, expandedUri, params = {}, timestamp = new Date()) {
    return {
        uriTemplate,
        expandedUri,
        params,
...
function createPromptGetInvocation(name, params = {}, timestamp = new Date()) {
    return {
        name,
        params,
        timestamp,
...
function createToolCallInvocation(toolName, success = true, params = {}, timestamp = new Date()) {
    return {
        toolName,
        params,
        timestamp,
...
describe("ContentCache", () => {
    ...
    describe("Resource caching", () => {
        it("should store and retrieve resource content", () => {
            ...
            expect(content && "text" in content ? content.text : undefined).toBe("Content for file:///test.txt");
        });
        ...
        it("should handle clearing non-existent resource", () => {
            expect(() => cache.clearResource("file:///nonexistent.txt")).not.toThrow();
        });
    });
    describe("Resource template caching", () => {
        it("should store and retrieve resource template content", () => {
            ...
            const invocation = createResourceTemplateReadInvocation(uriTemplate, expandedUri, params);
        ...
        it("should replace existing resource template content", () => {
            const uriTemplate = "file:///{path}";
            const invocation1 = createResourceTemplateReadInvocation(uriTemplate, "file:///test1.txt", { path: "test1.txt" }, new Date(1000));
            const invocation2 = createResourceTemplateReadInvocation(uriTemplate, "file:///test2.txt", { path: "test2.txt" }, new Date(2000));
        ...
        it("should clear specific resource template", () => {
            ...
            cache.setResourceTemplate(template1, createResourceTemplateReadInvocation(template1, "file:///test1.txt"));
            cache.setResourceTemplate(template2, createResourceTemplateReadInvocation(template2, "file:///test2.txt"));
        ...
        it("should handle clearing non-existent resource template", () => {
            expect(() => cache.clearResourceTemplate("file:///{nonexistent}")).not.toThrow();
        });
    });
    describe("Prompt caching", () => {
        it("should store and retrieve prompt content", () => {
            ...
            expect(messageContent && "text" in messageContent
                ? messageContent.text
                : undefined).toBe("Prompt content for testPrompt");
        });
        ...
        it("should replace existing prompt content", () => {
            const name = "testPrompt";
            const invocation1 = createPromptGetInvocation(name, { city: "NYC" }, new Date(1000));
            const invocation2 = createPromptGetInvocation(name, { city: "LA" }, new Date(2000));
    ...
    describe("Tool call result caching", () => {
        it("should store and retrieve successful tool call result", () => {
            ...
            expect(toolContent && "text" in toolContent ? toolContent.text : undefined).toBe("Result from testTool");
        });
        ...
        it("should replace existing tool call result", () => {
            const toolName = "testTool";
            const invocation1 = createToolCallInvocation(toolName, true, { arg1: "value1" }, new Date(1000));
            const invocation2 = createToolCallInvocation(toolName, true, { arg1: "value2" }, new Date(2000));
    ...
    describe("clearAll", () => {
        it("should clear all cached content", () => {
            // Populate all caches
            cache.setResource("file:///test.txt", createResourceReadInvocation("file:///test.txt"));
            cache.setResourceTemplate("file:///{path}", createResourceTemplateReadInvocation("file:///{path}", "file:///test.txt"));
    ...
    describe("clearByUri", () => {
        ...
        it("should clear resource template with matching expandedUri", () => {
            ...
            const expandedUri = "file:///test.txt";
            const params = { path: "test.txt" };
            cache.setResourceTemplate(uriTemplate, createResourceTemplateReadInvocation(uriTemplate, expandedUri, params));
        ...
        it("should clear both regular resource and resource template with same URI", () => {
            ...
            cache.setResource(uri, createResourceReadInvocation(uri));
            cache.setResourceTemplate(uriTemplate, createResourceTemplateReadInvocation(uriTemplate, uri, params));
            expect(cache.getResource(uri)).not.toBeNull();
            expect(cache.getResourceTemplate(uriTemplate)).not.toBeNull();
        ...
        it("should not clear resource template with different expandedUri", () => {
            ...
            cache.setResourceTemplate(uriTemplate, createResourceTemplateReadInvocation(uriTemplate, expandedUri1, params1));
            cache.setResourceTemplate("file:///{other}", createResourceTemplateReadInvocation("file:///{other}", expandedUri2, params2));
            // Clear by first URI
        ...
        it("should handle clearing non-existent URI", () => {
            expect(() => cache.clearResourceAndResourceTemplate("file:///nonexistent.txt")).not.toThrow();
        });
    });
    describe("Edge cases", () => {
...
//# sourceMappingURL=contentCache.test.js.map