...
describe("OAuth Scope Discovery", () => {
    ...
    it("should return scopes from resource metadata when available", async () => {
        ...
        const scopes = await discoverScopes("http://localhost:3000", resourceMetadata);
    ...
    it("should return undefined when discovery fails", async () => {
        const { discoverAuthorizationServerMetadata } = await import("@modelcontextprotocol/sdk/client/auth.js");
        vi.mocked(discoverAuthorizationServerMetadata).mockRejectedValue(new Error("Discovery failed"));
        const scopes = await discoverScopes("http://localhost:3000");
        expect(scopes).toBeUndefined();
    });
    ...
    it("should pass fetchFn to discoverAuthorizationServerMetadata when provided", async () => {
        ...
        expect(discoverAuthorizationServerMetadata).toHaveBeenCalledWith(new URL("/", "http://localhost:3000"), { fetchFn: mockFetchFn });
    });
});
