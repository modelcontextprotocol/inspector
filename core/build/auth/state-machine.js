...
import { discoverAuthorizationServerMetadata, registerClient, startAuthorization, exchangeAuthorization, discoverOAuthProtectedResourceMetadata, selectResourceURL, } from "@modelcontextprotocol/sdk/client/auth.js";
...
export const oauthTransitions = {
    metadata_discovery: {
        ...
        execute: async (context) => {
            ...
            const resource = resourceMetadata
                ? await selectResourceURL(context.serverUrl, context.provider, resourceMetadata)
                : undefined;
    ...
    client_registration: {
        ...
        execute: async (context) => {
            ...
            if (!context.provider.scope || context.provider.scope.trim() === "") {
                ...
                const scopesSupported = context.state.resourceMetadata?.scopes_supported ||
            ...
            let fullInformation = context.state.oauthClientInfo ??
                (await context.provider.clientInformation());
            if (!fullInformation) {
                // Check if provider has clientMetadataUrl (CIMD mode)
                const clientMetadataUrl = "clientMetadataUrl" in context.provider &&
    ...
    authorization_redirect: {
        ...
        execute: async (context) => {
            ...
            if (!scope || scope.trim() === "") {
                scope = await discoverScopes(context.serverUrl, context.state.resourceMetadata ?? undefined, context.fetchFn);
            }
            const providerState = context.provider.state();
            const state = await Promise.resolve(providerState);
            const { authorizationUrl, codeVerifier } = await startAuthorization(context.serverUrl, {
                metadata,
                clientInformation,
    ...
    token_request: {
        canTransition: async (context) => {
            ...
            const clientInfo = context.state.oauthClientInfo ??
                (await context.provider.clientInformation());
