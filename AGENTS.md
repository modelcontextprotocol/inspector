...
- ALWAYS do `npm run validate` before pushing any changes — from the repo root it mirrors what CI enforces. It runs four per-client validations followed by the cross-client smokes:
  - `validate:web` — web `format:check` + `lint` + `build` + `test:coverage`.
  - `test:cli` / `test:tui` — the CLI and TUI suites **with their coverage gates** (each runs the client's own `test:coverage`). Both build their client bundle as part of `test:coverage`, so the smokes below don't have to.
  - `validate:launcher` — now **self-contained**: `cd clients/launcher && npm run validate` (`format:check` + `lint` + `build` + `test:coverage` in its own folder). It has **no** dependency on the other clients being built; its only job is to validate the launcher itself.
  - `smoke` — the end-to-end smokes (`smoke:launcher` `--help` dispatch, plus the prod `smoke:cli` / `smoke:tui` / `smoke:web`). This step contains **no build commands**: by the time it runs, the prior validations have already built the cli/tui/launcher bundles (and `smoke:web` builds `clients/web/dist` on demand), so nothing is built twice.

  Each sub-script (`validate:web`, `test:cli`, `test:tui`, `validate:launcher`, `smoke`, and the individual `smoke:*`) can be run on its own when iterating — but a standalone `smoke` assumes the relevant client bundles are already built. Storybook is the only CI step left out (see below).
