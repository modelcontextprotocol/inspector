...
- **`src/utils/`** — pure, side-effect-free functions. Input → output, no DOM/browser/storage I/O, no subsystem ownership. Trivially unit-testable with no mocks. (Anchors: `jsonUtils`, `schemaUtils`, `toolUtils`, `maskSecrets`, `inspectorTabs`, `deepLink`, `mcpNetworkHeaders`.) This includes modules that are pure **shared domain types plus their pure constructors/transforms** (`customHeaders` — `CustomHeader` + `headersToRecord`/`migrateFromLegacyAuth`, a shape staged for `ServerSettingsForm`, see `specification/v2_ux_interfaces_plan.md`, so it currently has no importer but its own test); there is no `types/` sub-bucket **inside** `lib`/`utils` (the removal of `lib/types/` is what the `customHeaders` move settles). This is distinct from the top-level `src/types/`, which is not the place for new domain types — it's essentially the home for ambient `.d.ts` module stubs (e.g. the `react-syntax-highlighter` shims wired through `tsconfig.app.json` `paths`). (One plain-`.ts` domain type still sits there, the dead `navigation.ts` `InspectorTab`, superseding [#1785](https://github.com/modelcontextprotocol/inspector/issues/1785) will remove it — until then a pure domain type belongs in `utils/`, not `src/types/`.) **Diagnostic logging (`console.warn`/`console.error`) does not count as a side effect** for this rule — a validator that warns on bad input is still "pure" here (`sandbox-csp`, `jsonUtils`, `schemaUtils` all warn). Two forms of importing from `@inspector/core` are also fine: a **type-only** import is not a subsystem dependency (`pendingReauth` is pure type declarations), and **re-exporting pure functions or constants** from core is not subsystem ownership either (`oauthUx`/`oauthFlow` re-export core copy/predicates) — what makes a module `lib` is wrapping core's *stateful runtime*, not merely importing from it.
...
Nothing keys off the boundary (no path alias, no build/coverage glob) — it's a human-legible signal at import time, valuable in a codebase this test-heavy (the ≥90% per-file gate). When adding a module, place it by the rule; when it genuinely mixes both (e.g. `downloadFile` bundles DOM-side-effect helpers with a couple of pure ones), keep it whole on its dominant side (`lib`) rather than splitting hairs.
