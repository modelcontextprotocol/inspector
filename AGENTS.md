...
- Each client's `test:coverage` enforces a per-file gate: lines ≥ 90, statements ≥ 85, functions ≥ 80 (CI enforces this gate). The per-file `branches` floor is **70 for `clients/web`** and 50 for `clients/cli` / `clients/tui` / `clients/launcher`. Web's branch floor was raised from 50 → 70 after auditing every outlier (#1271): Mantine `useComputedColorScheme` dark-theme branches are now covered by mocking the hook, and dnd-kit `isDragging` branches by mocking `useSortable`. The remaining sub-100 branches in web are purely defensive guards (e.g. a `Select.onChange` receiving a value outside the allowed list) or happy-dom-inherent (portal mount points, `useMediaQuery` fallbacks, `typeof window` SSR fallbacks) and stay uncovered. New business-logic branches should still be covered.
