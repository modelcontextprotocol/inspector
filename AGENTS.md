...
- The repo root no longer has aggregate `test` / `test:coverage` scripts — each client self-validates, so run `npm run validate` from the root (all clients) or `cd clients/<name> && npm run validate` (one client). Each client still exposes its own `test` / `test:coverage` for quick iteration.
...
- ALWAYS do `npm run validate` before pushing any changes — from the repo root it mirrors what CI enforces. **`validate` is now nothing but the four per-client validations** (`validate:web` → `validate:cli` → `validate:tui` → `validate:launcher`); each delegates to that client's own `npm run validate`, which is uniformly `format:check` + `lint` + `build` + `test:coverage` in its own folder. So every client is self-validating and the top level just chains them — no cross-client build dependencies.
  - The one CLI nuance: `clients/cli`'s out-of-process `e2e.test.ts` spawns the built binary, so its `test:coverage` **builds first** (`test-servers:build && build && vitest`). To avoid building it twice, `clients/cli`'s `validate` folds that in — it is `format:check && lint && test:coverage` with **no** separate `build` step (the other clients, whose tests don't spawn their bundle, keep an explicit `build`). `validate:web`/`validate:tui`/`validate:launcher` are the uniform `format:check && lint && build && test:coverage`.
- **`smoke` is a separate top-level target, NOT part of `validate`.** Run it (or the individual `smoke:*`) after a build/validate: `npm run validate && npm run smoke`. It runs `smoke:launcher` (`--help` dispatch) plus the prod `smoke:cli` / `smoke:tui` / `smoke:web`, and contains **no build commands** — it assumes the cli/tui/launcher bundles already exist (a full `validate` builds them; `smoke:web` builds `clients/web/dist` on demand). CI runs `validate` then `smoke` as the step after it. Storybook is the only CI step left out (see below).
