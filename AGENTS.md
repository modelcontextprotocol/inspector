...
- **`src/utils/`** — pure, side-effect-free functions. Input → output, no DOM/browser/storage I/O, no subsystem ownership. Trivially unit-testable with no mocks. (Anchors: `jsonUtils`, `schemaUtils`, `toolUtils`, `maskSecrets`, `inspectorTabs`, `deepLink`, `sandbox-csp`, `customHeaders`.) This includes modules that are pure **shared domain types plus their pure constructors/transforms** (`customHeaders` — `CustomHeader` + `headersToRecord`/`migrateFromLegacyAuth`); there is no separate `types/` bucket. **Diagnostic logging (`console.warn`/`console.error`) does not count as a side effect** for this rule — a validator that warns on bad input is still "pure" here (`sandbox-csp`, `jsonUtils`, `schemaUtils` all warn). A module that only imports **types** from `@inspector/core` (or elsewhere) is likewise pure and belongs here — a type-only import is not a subsystem dependency (e.g. `pendingReauth` is pure type declarations, `oauthUx`/`oauthFlow` re-export core copy/predicates).
